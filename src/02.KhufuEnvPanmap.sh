...
cp "$1".fa "$id1".reordered.from"$id2".panmap.fa
...
cp "$1".fa "$id1".reassigned.from"$id2".panmap.fa
...
cp "$1".fa "$id"Smiss"$Smiss".panmap.fa 
...
############################
panmapGetROH(){
cat $1 | cut -f 1-2,5- | hapmapGetROH
}
############################
