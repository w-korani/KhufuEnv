...
cat $1  | hapmapGetSNP  > "$tmpDir0103"/hapmap
...
### bedtools getfasta -tab -fi $ref -bed "$tmpDir0103"/bed | tr ':' '\t' | tr '-' '\t' |  cut -f 1,3,4    > aaa
cat $2 | hapmap2txt > "$tmpDir0103"/alleles_ref0
cat "$tmpDir0103"/hapmap | sed "1d"  | cut -f 1-2 | tr '\t' '_' > "$tmpDir0103"/bed
merge "$tmpDir0103"/bed "$tmpDir0103"/alleles_ref0 | sed "s:NA$:-:g" | cut -f 2 > "$tmpDir0103"/alleles_ref
...
paste <(cat "$tmpDir0103"/hapmap | cut -f 3- | sed '1d' | tr '\t' ',' | awk 'function sortuniq (str){if(str==0 || str =="-"  ){return "-"}; nA=split(str,A,",");S=""; asort(A, B) ; for(a=1;a<=nA;++a){ if(B[a] !=0 && B[a] != s && B[a] != "-") {S=S","B[a]; s=B[a] }; s=B[a]};S=substr(S,2);if(S==""){S=0} ; return S } {print sortuniq($0) }' | tr ',' '/'  | paste - <(cat "$tmpDir0103"/hapmap | cut -f 3- | sed "s:[\t-]::g" | sed "1d" | perl -F -lane '{print sort@F}' | tr -s '[ACGT]' ) | awk '{if(length($2) ==2 ) {print substr($2,1,1)"/"substr($2,2,1)} else {print $1} }'|  sed '1iII') <(cat "$tmpDir0103"/hapmap | cut -f 1-2)        <(cat "$tmpDir0103"/hapmap | cut -f 3- | sed "s:,::g" | awk '{for( i=1;i<=NF;++i ){ s="";if($i=="-"){s="NN"} else if(length($i) >= 2){s=$i} else {s=$i$i} ; S=S","s   };S=substr(S,2); print S; S="" }'  | tr ',' '\t' )     | awk '{if(NR==1) {print "rs#\talleles\tchrom\tpos\tstrand\tassembly#\tcenter\tprotLSID\tassayLSID\tpanelLSID\tQCcode\t"$0} else {print $2"_"$3"\t"$1"\t"$2"\t"$3"\t+\tNA\tNA\tNA\tNA\tNA\tNA\t"$0} }' | cut -f 1-11,15- > "$tmpDir0103"/hapmap2
...
cat $1 > "$tmpDir0104"/hapmap
cat "$tmpDir0104"/hapmap | sed "1d" | cut -f 3- > "$tmpDir0104"/txt2
cat "$tmpDir0104"/txt2|  tr '\t' ',' | sed "s:-,::g" | sed "s:,-$::g" > "$tmpDir0104"/seq
cat "$tmpDir0104"/seq | sortUniqueValuesDS > "$tmpDir0104"/alleles

#cat "$tmpDir0104"/hap| sed "1d" | cut -f 1-2 | awk '{print $1"\t"$2-1"\t"$2}' > "$tmpDir0104"/bed
...
cat $2 | hapmap2txt > "$tmpDir0104"/alleles_ref0
cat "$tmpDir0104"/hapmap | sed "1d"  | cut -f 1-2 | tr '\t' '_' > "$tmpDir0104"/bed
merge "$tmpDir0104"/bed "$tmpDir0104"/alleles_ref0 | sed "s:NA$:-:g" | cut -f 2 > "$tmpDir0104"/alleles_ref
paste "$tmpDir0104"/alleles_ref "$tmpDir0104"/alleles |  awk '{nA=split($2,A,","); S=""; for(a=1;a<=nA;++a){if(A[a]!=$1){S=S","A[a]} }; S=substr(S,2); if(S==""){S="."} ; print $1"\t"S }'  > "$tmpDir0104"/refalt
...
cat "$tmpDir0104"/bed | tr '_' '\t' | awk '{print $1"\t"$2"\t."}' | paste - "$tmpDir0104"/refalt "$tmpDir0104"/dep | sed "1i#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT" > "$tmpDir0104"/info
mkdir "$tmpDir0104"/Geno
num=$(cat "$tmpDir0104"/hapmap | head -1 | wc -w)
for i in $(seq 3 $num)
do
   #echo $i
   paste <(paste "$tmpDir0104"/refalt | tr '\t' ',' | sed "1ialleles") <(cat "$tmpDir0104"/hapmap | cut -f "$i" ) | awk '{if(NR==1){print $2}else if($2=="-"){ print "." } else {nA=split($1,A,",");nB=split($2,B,","); S=""; for(a=1;a<=nA;++a){ for(b=1;b<=nA;++b){ if(A[a] == B[b] ){S=S","a-1} }  }; S=substr(S,2) ; print S }  }' > "$tmpDir0104"/Geno/$(echo $i | awk '{printf "%020d\n", $0}' )
done
...
cat $1 | grep -vw "-" | awk '{S=$1"\t"$2"\t"; if($0~","){S=S"0"}else{S=S"1"}; print S}' | awk '{if(NR==1){X=$2} else{ if($3!=homo){ print chr"\t"X1"\t"X"\t"X-X1+1"\t"cnt"\t"homo; X1=$2; cnt=1 }else{X0=$2;cnt+=1};homo=$3; X=$2; chr=$1} }END{ print chr"\t"X1"\t"X"\t"X-X1+1"\t"cnt"\t"homo }'  | sed 1d | awk -v min=$2 'BEGIN{if(min == ""){min=50}}{if($6==1 && $5 >= min) print $0}' |  cut -f 1-5 | grep -v "-"
}
############################
hapmapGetROH(){
...
do
   cat "$tmpFile0108" | cut -f 1-2,$i  | grep -vw "-" | hapmapGetROHforSingleSample - $2 | sed "s:^:${head[$((i-1))]}\t:g"
done
rm "$tmpFile0108"
